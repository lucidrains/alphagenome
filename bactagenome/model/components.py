...
publication_heads_config = {
    'human': {
        'organism': 'human',
        'num_tracks_1bp': 3165,
        # 1174 total RNA-seq + 961 polyA plus RNA-seq + 516 hCAGE + 30 LQhCAGE + 167 ATAC-seq + 305 DNase-seq + 12 PRO-cap
    ...
    'mouse': {
        'organism': 'mouse',
        'num_tracks_1bp': 730,
        # 168 total RNA-seq + 365 polyA plus RNA-seq + 172 hCAGE + 16 LQhCAGE + 18 ATAC-seq + 67 DNase-seq
...
class DownresBlock(Module):
    def __init__(
            ...
            dim,
            channels_to_add=128
            # this is new as well? instead of doubling channels, they add 128 at a time, and use padding or slicing for the residual
    ):
        super().__init__()
    ...
    def __init__(
        ...
        for layer_index in range(depth):
            ...
            if divisible_by(layer_index, self.pairwise_every):
                single_to_pairwise = SingleToPairwise(dim=dim, dim_pairwise=dim_pairwise,
                                                      heads=single_to_pairwise_heads, pool_size=pool_size)
...
class TransformerUnet(Module):
    ...
    def forward(
        ...
        return TransformerUnetOutput(out, single,
                                     pairwise)  # the final output, as well as single and pairwise repr from the transformer
...
class TargetScaler(Module):
    def __init__(
            ...
            # for rna-seq they squash the signal. not intimate enough with these assays to understand why yet
